...
comparison effects to a CLR comparer callback. An invalid key raises TypeError
only when it would actually be called, so empty input with a bad key succeeds
instead of failing.
