...
- [ ] **Commit 05 — Complete operator protocol dispatch.** Honor `__length_hint__` and `__index__` with CPython validation and fallback semantics. Addresses LY-MOD-017 and LY-MOD-036.
...
- [ ] **Commit 10 — Preserve atomic tuple identity in deep copies.** Reuse tuple inputs when every recursively copied element is unchanged, without regressing cycles or memo dictionaries. Addresses LY-MOD-004.
...
- [ ] **Commit 15 — Generalize recursive dataclass dictionary conversion.** Preserve supported key objects and recursively copy keys and values in `asdict`. Addresses LY-MOD-007.
...
- [ ] **Commit 24 — Share singledispatch's MRO resolver.** Make public `dispatch(cls)` select exactly as ordinary decorated invocation does. Addresses LY-MOD-031.
- [ ] **Commit 25 — Make `cmp_to_key` wrappers comparable.** Implement the comparator-backed rich comparison surface directly on wrapper values. Addresses LY-MOD-032.
