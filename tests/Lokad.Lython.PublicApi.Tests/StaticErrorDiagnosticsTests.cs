...
""");
    ...
    [Fact]
    public void BooleanLiteralsPassMoreIntegerContracts()
    {
        var compiled = new LythonEngine().Compile(
            """
import math, random, re, difflib
print(random.randint(True, 5))
print(re.sub("a", "b", "aaa", count=True))
print(difflib.get_close_matches("a", ["a", "b"], n=True))
print(difflib.HtmlDiff(tabsize=True) is not None)
print(random.randrange(True, 5) in [1, 2, 3, 4])
print(math.perm(5, True))
print(difflib.SequenceMatcher(None, "abcd", "abXcd").find_longest_match(True, 4, 0, 4))
""");
    ...
    [Fact]
    public void BooleanFalseStillFailsPositiveIntegerContracts()
    {
        var compiled = new LythonEngine().Compile(
            """
import difflib
print(difflib.get_close_matches("a", ["a", "b"], n=False))
""");

        Assert.False(compiled.IsValid);
        Assert.Single(compiled.Diagnostics, d => d.Code == "LA3158");
    }

    [Fact]
    public void BooleanLiteralsPassOptionalIntegerContracts()
    {
        var compiled = new LythonEngine().Compile(
            """
print("a,b,c".split(",", True))
print("aaa".replace("a", "b", True))
print("hello".find("l", True))
import random
print(random.choices([1, 2], k=True))
""");
