...
""",
    ...
    [Fact]
    public void SubprocessRun_RejectsOversizedCombinedCapturedHostOutput()
    {
        var host = new MockLythonHost();
        host.EnableSubprocess();
        host.SeedSubprocessResult(["t"], 0, "123456", "abcdef");
        ...
        Assert.False(result.Success);
        Assert.Equal("RuntimeError", result.Failure?.ExceptionType);
        Assert.Contains("combined captured output exceeded maximum bytes (10)", result.Failure?.Message, StringComparison.Ordinal);
    }
...
""",
    ...
    public void SubprocessRun_UsesContainedCwdAndEnvironmentWhenOmitted()
    {
        ...
        Assert.True(result.Success, DescribeFailure(result));
        var request = host.LastSubprocessRequest;
        Assert.NotNull(request);
        Assert.Equal("/repo", request.Cwd);
        Assert.NotNull(request.Environment);
        Assert.Equal("/bin", request.Environment["PATH"]);
...
""",
        ...
        Assert.NotNull(host.LastSubprocessRequest);
        Assert.Equal(LythonSubprocessInvocationMode.Shell, host.LastSubprocessRequest.InvocationMode);
...
""",
    ...
    [Fact]
    public void CompletedProcessAcceptsBooleanReturnCode()
    {
        var host = new MockLythonHost();
        host.EnableSubprocess();
        ...
        var result = new LythonEngine().Run(
            """
import subprocess
made = subprocess.CompletedProcess(["cmd"], True)
return str(made.returncode)
""",
            host);
