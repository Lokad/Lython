...
internal static partial class StaticContracts
{
    ...
    private static StaticCallableContract[] CreateCallableContracts()
    {
        var contracts = new List<StaticCallableContract>
        {
            ...
            new(AbstractValueKind.Path, "unlink", 0, 0, "LA3114", "Path.unlink() expects no arguments."),
            new(AbstractValueKind.Path, "mkdir", 0, 0, "LA3114", "Path.mkdir() expects no arguments."),
            new(AbstractValueKind.Path, "read_text", 0, 1, "LA3114", "Path.read_text([encoding]) expects zero or one argument.", ParameterNames: ["encoding"]),
            new(AbstractValueKind.Path, "open", 0, 2, "LA3114", "Path.open([mode][, encoding]) expects zero to two arguments.", ParameterNames: ["mode", "encoding"]),
            new(AbstractValueKind.Path, "write_text", 1, 3, "LA3114", "Path.write_text(text[, encoding][, newline]) expects one to three arguments.", ParameterNames: ["text", "encoding", "newline"]),
