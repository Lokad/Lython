...
internal sealed partial class LythonRuntime
{
    private static object Round(object[] arguments, LythonSourceSpan span, ExecutionContext context)
    {
        ...
        // User-defined __round__ takes precedence like CPython, receiving
        // ndigits untouched when present; anything it resolves to is called.
        if (arguments[0] is PyInstance roundInstance &&
            roundInstance.TryGetAttribute("__round__", context, span, out var roundMember))
        {
            if (roundMember is not ICallable roundCallable)
            {
                throw new LythonRuntimeException("TypeError", "'" + UnboundTypeMethod.PythonTypeName(roundMember, context) + "' object is not callable", span);
            }

            return hasDigits
                ? CallableInvocation.InvokeUnary(roundCallable, arguments[1], span, context)
                : roundCallable.Invoke([], span, context);
        }
